...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        String dictionaryColumns = cliArgs.switchValue("-dictionary", dictionaries);
        ...
        String[] dictColumns = dictionaryColumns.split(",");
    ...
    private static Set<String> getColumnsWithDictEncoding(int columns, String[] dictColumns, String[] typeArr) {
        Set<String> columnsTypesSet = new HashSet<>(dictColumns.length);

        for (String dictColumn : dictColumns) {
            columnsTypesSet.add(dictColumn.toUpperCase());
        }

        Set<String> columnsWithDictEncodingEnabled = new HashSet<>();

        for (int i = 0; i < columns; ++i) {
            String type = typeArr[i % typeArr.length];

            if (columnsTypesSet.contains(type.toUpperCase())) {
                columnsWithDictEncodingEnabled.add("f" + i);
            }
        }

        return columnsWithDictEncodingEnabled;
    }
    ...
    private static void writeToParquet(Schema schema,
                                       ...
                                       Set<String> columnsWithDictEncoding) {
        Path path = new Path(file);
        ParquetWriter<GenericData.Record> writer = null;
        try {
            AvroParquetWriter.Builder<GenericData.Record> recordBuilder = AvroParquetWriter.
    ...
    private static void enableDictionaryEncodingForIndividualColumns(AvroParquetWriter.Builder<GenericData.Record> recordBuilder,
                                                                     Set<String> columnsWithDictEncoding, int columns) {
        for (int i = 0; i < columns; i++) {
            String columnName = "f" + i;
            recordBuilder.withDictionaryEncoding(columnName, columnsWithDictEncoding.contains(columnName));
        }
    }
